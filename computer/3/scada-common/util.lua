...
function util.new_watchdog(timeout)
    local self = { timeout = timeout, wd_timer = os.startTimer(timeout) }
    ...
    function public.feed()
        public.cancel()
        self.wd_timer = os.startTimer(self.timeout)
    end
...
function util.new_clock(period)
    ...
    -- start the clock
    function public.start() self.timer = os.startTimer(self.period) end
